...
public class MediaGlue {
    ...
    private void checkRequiredProperties(MessageDTO receivedMessage, String msgType, String expectedMimetype) {
        ...
        if (receivedMessageFile != null) {
            if (receivedMessageFile.getUrl().isEmpty() || !receivedMessageFile.getUrl().startsWith("mxc://")) {
    ...
    public void sendAttachmentToChatWithMsgType(
        ...
        checkRequiredProperties(receivedMessage, msgType, mimetype);
        RoomDTO room = checkRoomMembershipStateInDirectChatOf(actorName);
        checkRoomVersion(room);
    }

    @Then("{string} receives the attachment {string} from {string} via Matrix protocol v1.11 as {string}")
    @Then("{string} empfängt das Attachment {string} von {string} über Matrix-Protokoll v1.11 als {string}")
    ...
    @SneakyThrows
    @Then("{string} receives the attachment {string} from {string} in the room {string} using matrix protocol v1.11 as {string}")
    @Then("{string} empfängt das Attachment {string} von {string} im Raum {string} über Matrix-Protokoll v1.11 als {string}")
    public void getAttachmentToRoomWithMsgType(String actorName, String file, String userName, String roomName, String msgType) {
        ...
        byte[] receivedMedia = actor.asksFor(downloadAuthenticatedMedia().withFileId(receivedMessage.getFileId()));
        ...
        assertThat(requireNonNull(receivedMessage.getFile()).getV()).isEqualTo(existingMessageFile.getV());
        checkRoomMembershipState(actor, roomName);
