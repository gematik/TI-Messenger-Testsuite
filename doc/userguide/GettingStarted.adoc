...
Test cases for dehydrated devices are now available.
...
For the provided Serenity report, the reference implementation is tested against the reference implementation. This is intended to support in your development, but *the format is not sufficient to meet the entry criteria*, as only one example per test case is executed in the report, while two examples are required for approval. The report is not always up to date and complete, although we strive to keep it up to date and complete.

This is the command we used to create the example Serenity report:
```shell
mvn -P oneonly,reference-ci,dont-break-preparation -DpoolGroupString=RU1 -Dcucumber.filter.tags=@Zul:Pro --no-transfer-progress
```
