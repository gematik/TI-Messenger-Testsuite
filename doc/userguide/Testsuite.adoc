...
Tags, die in der Source combine_items.json angegeben sind und ebenfalls aus einem Device ausgelesen werden sollen (definiert in der pom.xml), können miteinander verglichen werden.
...
Geben wir z.B. die Property ‚os‘ (wie im obigen Beispiel) mit einem entsprechenden Wert mit, so wird geprüft, ob das Device hinter der URL des Testtreibers tatsächlich dieses ‚os‘ hinterlegt hat, wenn diese Property ebenfalls in der pom.xml definiert ist.
...
Dies kann auch global in der pom.xml geschehen, sodass dann an die Schnittstelle in der combine_items.json lediglich die ID weiter gegeben werden muss.
