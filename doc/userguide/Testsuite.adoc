...
Eine unterstützende Anleitung hierzu befindet sich link:GettingStarted.adoc#_zertifikate_einrichten[hier].
...
Mit Hilfe von Filtern, die direkt im .cute-File (später feature-File) gesetzt werden, kann nun diese Liste verkleinert und fokussiert werden.
...
Tags, die in der Source combine_itmes.json angegeben sind und ebenfalls aus einem Device ausgelesen werden sollen (definiert in der pom.xml), können miteinander verglichen werden.
Dies geschieht case sensitive (1:1 Beziehung).
Sollte sich der Tag in der combine_items.json und von dem in der pom.xml definierten unterscheiden, wird der Tag aus der Source combine_items.json als zusätzlicher Tag gewertet und in die Target combine_items.json übernommen.
...
Geben wir z.B. die Property ‚os‘ (wie im obrigen Beispiel) mit einem entsprechenden Wert mit, so wird geprüft, ob das Device hinter der URL des Testtreibers tatsächlich dieses ‚os‘ hinterlegt hat, wenn diese Property ebenfalls in der pom.xml definiert ist.
...
Die Daten, die von dem jeweiligen Device ausgelesen werden und die combine_items.json im Target Folder (target/generated-combine/combine_items.json) anreichern, werden in der pom.xml unter ‚cucumber-test-combinations-maven-plugin‘ definiert.
...
[TIP]
[black]#*_In der Target combine_items.json (target/generated-combine/combine_items.json) sind dann alle Schnittstellen zu den Testtreibern der einzelnen Devices inkl aller notwendigen Daten hinterlegt.
Mit Hilfe dieser Daten können dann die Testdaten für die Feature Files erzeugt und die einzelnen Tests ausgeführt werden._*#
...
Es können auch dedizierte Vergleichswerte definiert werden, die dann einzelnen Schnitstellen in der combine_items.json zugeordnet werden können.
Dies kann auch global in der pom.xml geschehen, so dass dann an die Schnitstellt in der combine_items.json lediglich die ID weiter gegeben werden muss.
...
Um einen Versionsfilter *global* zu setzen, muss der entsprechende Wert in die pom.xml eingetragen werden.
...
Für „ApiName2“ wird dann entweder der global gesetzte Filter in der pom.xml genutzt oder sollte dieser nicht befüllt sein, eben alle anderen Schnittstellen, da hier keine Beschränkung vorliegt.
...
Dann greifen die Default-Werte (amount=0 und default Matching Strategie aus der pom.xml)
...
Kann überschritten werden wenn Server nicht antwortet.
...
Für das erfolreiche Ausführen der Testfälle aus dem Folder "Zulasungstests" müssen auf Seiten des Anbieters folgende Clients vorhanden sein:
...
[TIP]
[black]#*_Zur Bereitstellung der Clients seitens des Herstellers ist Kapitel 4 nochmal zu erwähnen. Die obrige Aufzählung dient der Übersichtlichkeit._*#
...
Die Combine Items kann dann z.B. wie folgt aussehen, wenn wir hier davon ausgehen, dass "A" der Hersteller ist und "B" z.B. die
Referenzimplementierung oder zu Testzwecken eine weitere Instanz.

.Abbildung{counter:my-example-number: 1}: Beispiel Combine Items Zulasungstests
image:Combine_Zulass.png[float="center"]

image::Testrun.png[float="left"]

==== 9.3 Zusätzliche Tests - ZusatztestsOrgOnly

{empty} +

[TIP]
[black]#*_Diese zusätzlichen "ZusatztestsOrgOnly" Tests sind ausschließlich für die Zulassung von Clients, die nur eine Org-User Funktionalität haben. Für eine Zulassung eines TI-Messenger benötigen wir immer mindestens einen Client der auch die Funktionalität HBA-User hat. Die Zulassung einzelner Org-User Client kann nur im Anschluß einer HBA-User Client Zulassung durchgeführt werden._*#

Der neue Folder src/test/resources/templates/FeatureFiles/TI-M_11X/ZusatztestsOrgOnly ist entstanden, um die Möglichkeit zu haben, auch die Funktionalität eines
Client ohne die Nutzung der HBA Funktionalität, also der Rolle Organisations User, komplett testen zu können.

In diesem Folder sind all die Testfälle enthalten und für ihren Zweck angepasst, deren Funktionalität in der regulären Suite nur über die Clients mit HBA Funktionalität und dessen Gebrauch abgebildet wurden.

In Kombination mit den Testfällen der regulären Suite (mit dem Tag OrgOnly versehen) bildet ZusatztestsOrgOnly ein Testset ab, welches alle geforderten Funktionalitäten der normalen Suite beinhaltet und
die Möglichkeit bietet, einen Client abseits der HBA Funktionalität umfänglich zu testen. Dieser Client darf bei der Abfrage
der Schnittstelle nicht den Tag 'practitioner' besitzen bzw. muss info.app.clientInfo.canAdministrateFhirPractitioner=false sein.

Für das erfolgreiche Ausführen dieser Testfälle müssen auf der Seite des Anbieters folgende Clients vorhanden sein:

- 1 Client mit HBA Funktionalität
- 1 Client ohne HBA Funktionalität/ohne den Tag 'practitioner' (Testobjekt)
- 1 Org-Admin
...
[TIP]
[black]#*_Für die Bereitstellung der Clients seitens des Herstellers ist das Kapitel 4 nochmal zu erwähnen. Die obrige Aufzählung dient der Übersichtlichkeit._*#

In der Combine Items müssen die Tags "new" (für das Testobjekt) und "preIOP" (für alle Clients) wie folgt vergeben werden (die einzelnen Schnittstellen müssen dann auch in der tiger.yml eingetragen sein),
wenn wir hier auch davon ausgehen, dass "A" der Hersteller ist und "B" z.B. die Referenzimplementierung oder zu Testzwecken eine weitere Instanz.

.Abbildung{counter:my-example-number: 1}: Beispiel Combine Items ZusatztestsOrgOnly
image:Combine_OrgOnly.png[float="center"]

Der "new" Tag beschreibt hier das eigentliche Testobjekt und gibt später die Möglich explizit nur Kombinationen zu testen, wo das Testobjekt enthalten ist.
Andere Kombinationen werden, wo das "new" Testobjekt nicht enthalten ist, herausgefiltert.
Der Tag "preIOP" dient dazu alle anderen Clients in den IOP Tests auch gegen das Testobjekt mit dem "new" Tag zu testen und die entsprechenden Kombinationen zu bilden.

[TIP]
[black]#*_Weitere Informationen zu dem Nutzen und Verhalten der Tags sind in Kapitel 6.3 nachzulesen._*#

Eine weitere Einstellung die getätigt werden muss ist die folgende:

In der pom.xml muss der Pfad angepasst werden, der auf die zu nutzenden Feature Files zielt.

Default ist:

.Abbildung{counter:my-example-number: 1}: Auszug pom.xml - Default
image:TempDir_Zulass.png[float="center"]

Der Eintrag muss wie folgt geändert werden, damit auch der ZusatztestsOrgOnly Folder mit in den Testrun einbezogen wird:

.Abbildung{counter:my-example-number: 1}: Auszug pom.xml - Änderung
image:TempDir_OrgOnly.png[float="center"]

Wenn diese Änderung temporär vorgenommen wurden, kann ein Testrun mit diesem Befehlt gestartet werden:

mvn clean install -Dcucumber.filter.tags=@Ctl:OrgOnly -Pprod,newonly,dont-break-preperation

Der Paramerter kann auch beim Starten mit angegeben werden:

-Dfeature.template.dir=<projectdir>/src/test/resources/templates/FeatureFiles/TI-M_11X

Das Profil "dont-break-preperation" muss hier genutzt werden, da nur für spezifische TestfällKombinationen (mit dem Tag OrgOnly und nur Kombinationen wo der Tag "new" setzt wurde)
gebildet werden. Da andere Tabellen leer bleiben, würde ohne dieses Profil der ganze Testrun direkt nach dem Start abbrechen!
...
So ist schon direkt zu sehen, welche Schnittstellen an diesem Test Run beteilig waren.
...
Unterhlab dieser Darstellung sind alle Tags, die in dem Testrun bzw der Testfälle vorhanden waren, aufgeführt.
