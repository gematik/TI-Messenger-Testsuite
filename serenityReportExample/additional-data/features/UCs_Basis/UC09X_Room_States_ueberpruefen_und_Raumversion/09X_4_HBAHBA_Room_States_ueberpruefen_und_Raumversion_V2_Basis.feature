# language: de
@File:FeatureFile_09X_04_V2_Basis @Ctl:UseCaseV2_09X_Basis @PRODUKT:TI-M_Client_Basis @PRODUKT:TI-M_FD_Basis @Zul:Pro @AFO-ID:A_25813-01 @AFO-ID:A_25814-01 @AFO-ID:A_25426 @AFO-ID:A_26248 @AFO-ID:A_26338-01 @NB:JA
Funktionalität: 09X. (4) Room States (Basis Spec)
  ...
  @Ctl:AllowAll @Ctl:RoomStates @Ctl:RoomVersion @TCID:TIM_V2_BASIS_AF_09X0401 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  Szenariogrundriss: 09X.04.01 Room States - Chat - Default Room State (HBA-User an HBA-User)
    ...
    @Plugin:Shuffle(true) @Plugin:MaxSameColumnProperty(ApiName1,homeserver,1) @Plugin:DistinctProperty(homeserver) @Plugin:AllowSelfCombine(false) @Plugin:AllowDoubleLineup(true) @Plugin:Filter(ApiName1.hasTag("proClient")) @Plugin:Filter(ApiName2.hasTag("proClient")) @Plugin:Filter(ApiName1.hasTag("practitioner")) @Plugin:Filter(ApiName2.hasTag("practitioner"))
    Beispiele:
      | ApiName1              | ApiName2              |
      | RU2-Ref-Pract-SDK-HS3 | RU2-Ref-Pract-SDK-HS4 |

  @Ctl:AllowAll @Ctl:RoomStates @Ctl:RoomVersion @TCID:TIM_V2_BASIS_AF_09X0402 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
