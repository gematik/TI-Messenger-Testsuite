# language: de
@File:FeatureFile_05_02_V2_Basis @Ctl:UseCaseV2_05_Basis @PRODUKT:TI-M_Client_Basis @PRODUKT:TI-M_FD_Basis @PRODUKT:VZD_FHIR @Zul:Pro @Zul:ProKK @AF-ID:AF_10061-04 @AFO-ID:A_25045-01 @AFO-ID:A_25046 @NB:JA
Funktionalität: 5.1.5 (2) Einladung von Akteuren außerhalb einer Organisation (Basis Spec)
  ...
  @Ctl:AllowAll @Ctl:VZD @Ctl:BP @TCID:TIM_V2_BASIS_AF_050201 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  Szenariogrundriss: 05.02.01 Einladung außerhalb einer Organisation - Chat - User sucht anderen HBA-User
    ...
    @Plugin:Shuffle(true) @Plugin:Filter(!ApiName1.properties["homeserver"].equals(ApiName2.properties["homeserver"])) @Plugin:AllowSelfCombine(false) @Plugin:AllowDoubleLineup(true) @Plugin:Filter(ApiName1.hasTag("proClient")) @Plugin:Filter(ApiName2.hasTag("proClient")) @Plugin:Filter(ApiName2.hasTag("practitioner"))
    Beispiele:
      | ApiName1              | ApiName2              |
      | RU2-Ref-Pract-SDK-HS4 | RU2-Ref-Pract-SDK-HS3 |

  @Ctl:AllowAll @Ctl:VZD @Ctl:BP @TCID:TIM_V2_BASIS_AF_050202 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
