# language: de
@File:FeatureFile_06_04_V2_Basis @Ctl:UseCaseV2_06_Basis @PRODUKT:TI-M_Client_Basis @PRODUKT:TI-M_FD_Basis @PRODUKT:VZD_FHIR @Zul:Pro @AF-ID:AF_10062-03 @NB:JA
Funktionalität: 5.1.6 (4) Austausch von Events zwischen Akteuren außerhalb einer Organisation (Basis Spec)
  ...
  @Ctl:AllowAll @Ctl:VZD @TCID:TIM_V2_BASIS_AF_060401 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  Szenariogrundriss: 06.04.01 Austausch von Events außerhalb einer Organisation - Chat - AllowAll - HBA User schreibt HBA User
    ...
    @Plugin:Shuffle(true) @Plugin:DistinctProperty(homeserver) @Plugin:AllowSelfCombine(false) @Plugin:AllowDoubleLineup(true) @Plugin:Filter(ApiName1A.hasTag("proClient")) @Plugin:Filter(ApiName1B.hasTag("proClient")) @Plugin:Filter(ApiName1A.hasTag("practitioner")) @Plugin:Filter(ApiName1B.hasTag("practitioner"))
    Beispiele:
      | ApiName1A             | ApiName1B             |
      | RU2-Ref-Pract-SDK-HS4 | RU2-Ref-Pract-SDK-HS3 |

  @Ctl:AllowAll @Ctl:VZD @TCID:TIM_V2_BASIS_AF_060402 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
