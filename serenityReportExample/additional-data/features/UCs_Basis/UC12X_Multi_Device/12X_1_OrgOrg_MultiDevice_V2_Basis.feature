# language: de
@File:FeatureFile_12X_01_V2_Basis @Ctl:UseCaseV2_12X_Basis @PRODUKT:TI-M_Client_Pro @Zul:Pro @Zul:ProKK @AFO-ID:A_25514 @NB:ONEONLY
Funktionalität: 12X. (1) Nutzung von mehreren Geräten (Basis Spec)
  ...
  @Ctl:AllowAll @Ctl:OrgAdmin @Ctl:VZD @Ctl:MultiDevices @TCID:TIM_V2_Basis_AF_12X0101 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  Szenariogrundriss: 12X.01.01 Multi Device - Events außerhalb einer Organisation - Chat - Test-Client "A" mit 2 Devices
    ...
    @Plugin:Shuffle(true) @Plugin:Filter(ApiName1A.properties["homeserver"].equals(ApiName1B.properties["homeserver"])) @Plugin:Filter(ApiName1A.properties["homeserver"].equals(ApiName1C.properties["homeserver"])) @Plugin:Filter(!ApiName1A.properties["homeserver"].equals(ApiName2.properties["homeserver"])) @Plugin:AllowSelfCombine(true) @Plugin:AllowDoubleLineup(true) @Plugin:Filter(ApiName1A.hasTag("orgAdmin")) @Plugin:Filter(ApiName1B.hasTag("proClient")) @Plugin:Filter(ApiName1C.hasTag("proClient")) @Plugin:Filter(ApiName2.hasTag("proClient"))
    Beispiele:
  ...
  @Ctl:AllowAll @Ctl:OrgAdmin @Ctl:VZD @Ctl:MultiDevices @TCID:TIM_V2_Basis_AF_12X0102 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  ...
  @Ctl:AllowAll @Ctl:OrgAdmin @Ctl:VZD @Ctl:MultiDevices @TCID:TIM_V2_Basis_AF_12X0103 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  Szenariogrundriss: 12X.01.03 Multi Device - Events innerhalb einer Organisation - Chat - Test-Client "A" mit 2 Devices
    ...
    # @DistinctColumn(ApiName1C)
    # @MaxSameColumnProperty(ApiName1C,homeserver,2)
    @Plugin:Shuffle(true) @Plugin:EqualProperty(homeserver) @Plugin:AllowSelfCombine(true) @Plugin:AllowDoubleLineup(true) @Plugin:Filter(ApiName1A.hasTag("orgAdmin")) @Plugin:Filter(ApiName1B.hasTag("proClient")) @Plugin:Filter(ApiName1C.hasTag("proClient")) @Plugin:Filter(ApiName1D.hasTag("proClient"))
    Beispiele:
  ...
  @Ctl:AllowAll @Ctl:OrgAdmin @Ctl:VZD @Ctl:MultiDevices @TCID:TIM_V2_Basis_AF_12X0104 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
