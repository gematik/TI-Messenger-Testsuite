# language: de
@File:FeatureFile_01_01_V2_Pro @Ctl:UseCaseV2_01_Pro @Ctl:OneHomeServer @PRODUKT:TI-M_Client_Pro @PRODUKT:TI-M_FD_Pro @PRODUKT:VZD_FHIR @Zul:Pro @Zul:ProKK @AF-ID:AF_10059-02 @AFO-ID:A_25546 @NB:JA
Funktionalität: 5.1.1 (1) Organisationsressourcen im Verzeichnisdienst hinzufügen
  ...
  @Ctl:AllowAll @Ctl:OrgAdmin @Ctl:VZD @TCID:TIM_V2_PRO_AF_010101 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  ...
  @Ctl:AllowAll @Ctl:OrgAdmin @Ctl:VZD @TCID:TIM_V2_PRO_AF_010102 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  Szenariogrundriss: 01.01.02 Organisationsressourcen im Verzeichnisdienst hinzufügen - Healthcare-Service durch Org-Admin anlegen und MXID im Endpoint ändern
    ...
    @Plugin:Shuffle(true) @Plugin:EqualProperty(homeserver) @Plugin:AllowSelfCombine(true) @Plugin:AllowDoubleLineup(false) @Plugin:Filter(ApiName1A.hasTag("orgAdmin")) @Plugin:Filter(ApiName1B.hasTag("proClient")) @Plugin:Filter(ApiName1C.hasTag("proClient")) @Plugin:Filter(ApiName1D.hasTag("proClient"))
    Beispiele:
      | ApiName1A           | ApiName1B             | ApiName1C             | ApiName1D             |
      | RU2-Ref-ORG-Web-HS4 | RU2-Ref-Pract-SDK-HS4 | RU2-Ref-Pract-SDK-HS4 | RU2-Ref-Pract-SDK-HS4 |

  @Ctl:OrgAdmin @Ctl:VZD @TCID:TIM_V2_PRO_AF_010103 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  Szenariogrundriss: 01.01.03 Organisationsressourcen im Verzeichnisdienst hinzufügen - Healthcare-Service durch Org-Admin anlegen/ändern, Endpoint anlegen/ändern und löschen
    ...
    Dann existiert kein Endpoint von "B" für den Healthcare-Service "HealthcareServiceName" [Retry 6 - 1]
    Wenn "A" löscht den Healthcare-Service "HealthcareServiceName"
    Dann existiert der zuletzt gelöschte Healthcare-Service nicht mehr [Retry 6 - 1]

    # @MaxSameColumnProperty(ApiName1B,homeserver,2) #
    @Plugin:Shuffle(true) @Plugin:EqualProperty(homeserver) @Plugin:AllowSelfCombine(true) @Plugin:AllowDoubleLineup(false) @Plugin:Filter(ApiName1A.hasTag("orgAdmin")) @Plugin:Filter(ApiName1B.hasTag("proClient"))
    Beispiele:
      | ApiName1A           | ApiName1B             |
      | RU2-Ref-ORG-Web-HS4 | RU2-Ref-Pract-SDK-HS4 |

  @Ctl:OrgAdmin @Ctl:VZD @TCID:TIM_V2_PRO_AF_010104 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  Szenariogrundriss: 01.01.04 Organisationsressourcen im Verzeichnisdienst hinzufügen - Healthcare-Service durch Org-Admin mit zwei Endpoints anlegen und löschen
    ...
    Dann existiert kein Endpoint von "C" für den Healthcare-Service "HealthcareServiceName" [Retry 6 - 1]
    Wenn "A" löscht den Healthcare-Service "HealthcareServiceName"
    Dann existiert der zuletzt gelöschte Healthcare-Service nicht mehr [Retry 6 - 1]
  ...
  @Ctl:OrgAdmin @Ctl:VZD @TCID:TIM_V2_PRO_AF_010105 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
  ...
  @Ctl:AllowAll @Ctl:OrgAdmin @Ctl:VZD @TCID:TIM_V2_PRO_AF_010106 @PRIO:1 @TESTFALL:Positiv @STATUS:Implementiert
